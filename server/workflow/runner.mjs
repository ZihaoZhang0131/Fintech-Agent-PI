  ...
  async function executeNodeInner({ run, node, attempt, signal, store, update }) {
    ...
    tools.push({
      ...
      execute: async (_id, args) => {
        ...
        const kamiArtifacts = store.get(run.id).operations
          .filter(o => o.attemptId === attempt.id && o.tool === "render_kami_artifact" && o.status === "completed" && o.result?.details?.kind === "kami_artifact")
          .flatMap(o => [...Object.values(o.result.details.paths ?? {}), ...(o.result.details.previewPaths ?? [])].filter(p => typeof p === "string"));
        args = { ...args, artifacts: [...new Set([...args.artifacts, ...documentArtifacts, ...kamiArtifacts])] };
